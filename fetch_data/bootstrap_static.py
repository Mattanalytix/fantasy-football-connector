...
class BootstrapStaticFetcher:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 503:
                    logging.error("Service Unavailable (503) - The game may be updating.")
    ...
    def convert_table_to_polars(self, table_name, data):
        ...
        if table_name not in self.tables_to_extract:
            raise ValueError(f"Table '{table_name}' is not in the list of extracted tables.")
        
        # Convert the table to Polars DataFrame
        try:
            return pl.DataFrame(data[table_name])
        except Exception as e:
            logging.error(f"Failed to convert table '{table_name}' to Polars DataFrame: {e}")
            raise
